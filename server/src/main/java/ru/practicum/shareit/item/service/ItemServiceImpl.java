        ...
        if (item.getOwner().getId().equals(userId)) {
            itemDtoResponse.setLastBooking(mapper
                    .mapToBookingShortDto(bookings
                            .findFirstByItemAndStatusIsOrderByStartAsc(item,LocalDateTime.now(),Status.APPROVED).orElse(null)
                    ));
            itemDtoResponse.setNextBooking(mapper.mapToBookingShortDto(bookings
                    .findFirstByItemAndStatusIsOrderByEndDesc(item, LocalDateTime.now(), Status.APPROVED).orElse(null)));
    ...
    public ItemListDto getPersonalItems(Pageable pageable, Long userId) {
        ...
        for (Item item : findItems) {
            ...
            itemDtoResponse.setLastBooking(mapper.mapToBookingShortDto(
                    bookings.findFirstByItemAndStatusIsOrderByStartAsc(item, LocalDateTime.now(), Status.APPROVED).orElse(null)));
            itemDtoResponse.setNextBooking(mapper.mapToBookingShortDto(
                    bookings.findFirstByItemAndStatusIsOrderByEndDesc(item, LocalDateTime.now(), Status.APPROVED).orElse(null)));
